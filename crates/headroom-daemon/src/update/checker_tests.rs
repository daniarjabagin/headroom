...
use super::*;
use crate::testing::{Harness, eventually, harness};
use crate::update::{Install, InstallKind, Packager, Release, Version};
...
impl FakeFeed {
    fn new(script: Vec<Answer>) -> Arc<FakeFeed> {
        Arc::new(FakeFeed {
    ...
    fn gaps_from(&self, start: Instant) -> Vec<u64> {
...
async fn until_calls(feed: &FakeFeed, count: usize) {
    for _ in 0..5 * 24 * 60 {
        if feed.calls() >= count {
            return;
        }
...
fn modified(etag: &str) -> FeedResponse {
...
fn start(harness: &Harness, feed: Arc<FakeFeed>, current: &str) -> JoinHandle<()> {
    let config = UpdateConfig {
        feed,
        install: Install::Script,
        current: current.parse::<Version>().unwrap(),
    };
    tokio::spawn(run(harness.core.clone(), config))
}

fn stored(harness: &Harness) -> Option<CheckRecord> {
    harness
        .storage
