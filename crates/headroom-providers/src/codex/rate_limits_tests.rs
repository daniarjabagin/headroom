...
fn newest_main_snapshot_becomes_local_log_limits() {
    ...
    let snapshot = latest_snapshot(home.path(), identity(), now)
        .unwrap()
        .unwrap();
...
fn window_that_already_reset_is_shown_empty() {
    ...
    let snapshot = latest_snapshot(home.path(), identity(), after_reset)
        .unwrap()
        .unwrap();
...
fn legacy_snapshot_without_reset_time_still_classifies_windows() {
    ...
    let snapshot = latest_snapshot(home.path(), identity(), at("2026-07-10T09:00:00Z"))
        .unwrap()
        .unwrap();
...
fn picks_newest_observation_across_files() {
    ...
    let found = latest_observation(home.path()).unwrap().unwrap();
...
fn no_logs_means_no_snapshot() {
    let home = tempfile::tempdir().unwrap();
    assert_eq!(
        latest_snapshot(home.path(), identity(), at("2026-09-23T10:00:00Z")).unwrap(),
        None
    );
    write_rollout(
        home.path(),
    ...
    assert_eq!(latest_observation(home.path()).unwrap(), None);
}
