...
use crate::ui::widgets::{icon, svg_image};
...
pub fn link_icon(color: &str, kind: LinkKind, size: i32) -> gtk::Image {
    match kind {
        LinkKind::Status => svg_image(STATUS_SVG, color, size, &["headroom-link-glyph"]),
        LinkKind::Usage => svg_image(USAGE_SVG, color, size, &["headroom-link-glyph"]),
