...
| `onboarding.completed` | bool | `false` | Since 0.6.0. `false` until the user finishes (or skips for good) the first-run window. Settings stored by a daemon older than 0.6.0 load with `true`, and a database that an older daemon already used (it has accounts, log cursors or usage events) but that never stored settings is upgraded to `true` as well, so existing users never see onboarding; a fresh install starts with `false`. `ResetSettings` keeps it. |
